...
#define PLT_TARGET_SIZE	sizeof(u64)
#define PLT_INSNS 	3
...
static void build_plt(struct jit_ctx *ctx, bool write_target)
{
	...
	emit(A64_LDR64LIT(tmp, 8), ctx);
	emit(A64_BR(tmp), ctx);

	if (write_target)
		*(u64 *)(ctx->image + ctx->idx) = (u64)&dummy_tramp;
}
